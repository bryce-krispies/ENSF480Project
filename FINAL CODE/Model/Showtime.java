...
public class Showtime implements Serializable{
    ...
    public void setSeats() {
        ...
        for (; i < (int)(numOfCustomers * 0.9); i++) {
            Seat seat = new Seat(tickets.get(i), alphabet[i] + Integer.toString(i+1), Seat.type.forEveryone, true);
            seats.add(seat);
            tickets.get(i).setSeat(seat);
        }
        
        for (; i < numOfCustomers; i++) {
            Seat seat = new Seat(tickets.get(i), alphabet[i] + Integer.toString(i+1), Seat.type.forRegUser, true);
            seats.add(seat);
            tickets.get(i).setSeat(seat);
        }
    }
